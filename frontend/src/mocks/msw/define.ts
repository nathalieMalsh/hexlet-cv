import { http, HttpResponse, type DefaultBodyType, type Path } from 'msw'
...
export const defineGet = (path: Path, handler: GetHandler) => {
  return http.get(path, ({ request, cookies }) => {
    const ctx = createCtx(request, { cookies })
    console.log('[defineGet] request.url =', request.url)
    console.log('[defineGet] cookies arg =', cookies)
    console.log('[defineGet] ctx.cookies =', ctx.cookies)
    const piped = runPipeline(middlewares, (c, r) => {
      console.log('MSW handler hit:', request.method, request.url, cookies)
      return handler(c, r)
    })
...
export const definePost = <TBody>(path: Path, handler: PostHandler<TBody>) => {
  return http.post(path, async ({ request, cookies }) => {
    const ctx = createCtx(request, { cookies })
    console.log('[definePost] request.url =', request.url)
    console.log('[definePost] cookies arg =', cookies)
    console.log('[definePost] ctx.cookies =', ctx.cookies)
